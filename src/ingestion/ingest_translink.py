...
# Directory Output 
OUTPUT_DIR: Path = Path(__file__).parent.parent.parent / "data" / "bronze" / "translink_stops"
...
def ingest_translink_stops() -> Path: 
    ...
    stops_df.to_parquet(output_path, engine="pyarrow", compression="snappy", index=False)
    ...
    skytrain_df.to_parquet(skytrain_path, engine="pyarrow", compression="snappy", index=False)

    logger.info("Saved SkyTrain stations Parquet: %s (%d stations)", skytrain_path, len(skytrain_df))
